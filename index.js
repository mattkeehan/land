...
const badArgs = 'Landtree requires exactly 2 command line arguments: --mode=from_root, and a known company ID';

const getLandParcels = (companyId) =>  landParcels.filter(parcel => parcel.companyId === companyId).length;
...
const getParent = (company, trace) => {
  ...
  console.log(`${parent.id}; ${parent.name}; owner of ${getLandParcels(parent.id)} land parcels`);
  trace.pop();
...
const displayChildren = (company, depth, trace) => {
  ...
  children.forEach(child => {
    console.log(`${indent}- ${child.id}; ${child.name}; owner of ${getLandParcels(child.id)} land parcels`);
...
try {
  ...
  if (args[0] === '--mode=from_root' && args[1]) return getLandTreeFromRoot(args[1]);
